...
## 0.8.0-rc.1 - 本地发布候选（未发布）

- 把产品名统一为“AI 视频频道生产系统”，形成阶段2–7能力的本地可发布 RC。
- 增加在线／离线运行时策略、幂等重装、旧 Beta 升级、事务式自动回滚、修复和手动回滚。
- 修复卸载可能删除默认用户数据的问题；新安装改为程序／数据分离，旧原位数据保持兼容且不自动迁移。
- 增加带逐文件与聚合 SHA-256 的用户数据备份／恢复，以及 ZIP 路径穿越、重复项、重解析点和敏感文件名检查。
- 增加真实隔离 Codex home、Unicode／空格路径、三市场 recorded synthetic 端到端链、幂等／重启／频道隔离验证。
- 增加可复现 RC ZIP、内部资产清单、`SHA256SUMS.txt`、敏感信息扫描、版本索引和机器可读审批清单。
- 真实 OAuth、上传、Publication Receipt、Studio 数据、正式数据迁移、长期学习、GitHub push/tag/Release 和正式 EXE 覆盖全部未执行。
