...
$manifestPath = Join-Path $assetFull "unified-release-v0.12.0-rc.1.json"
...
foreach ($metadataName in @("unified-release-v0.12.0-rc.1.json", "SHA256SUMS.txt")) {
...
try {
    ...
    if ($null -eq $release) {
        $notesPath = Join-Path $repositoryRoot "docs\release-notes-v0.12.0-rc.1.md"
