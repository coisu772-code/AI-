...
$manifestPath = Join-Path $assetFull "unified-release-v0.16.0-rc.2.json"
...
function Invoke-GitHubJson {
    ...
    if ($null -ne $Body) {
        ...
        $parameters.Body = $serializableBody | ConvertTo-Json -Depth 20 -Compress
...
foreach ($metadataName in @("unified-release-v0.16.0-rc.2.json", "SHA256SUMS.txt")) {
...
try {
    ...
    if ($null -eq $release) {
        $notesPath = Join-Path $repositoryRoot "docs\release-notes-v0.16.0-rc.2.md"
