...
$manifestPath = Join-Path $assetFull "unified-release-v0.11.0-rc.4.json"
...
foreach ($metadataName in @("unified-release-v0.11.0-rc.4.json", "SHA256SUMS.txt")) {
...
try {
    ...
    if ($null -eq $release) {
        $notesPath = Join-Path $repositoryRoot "docs\release-notes-v0.11.0-rc.4.md"
