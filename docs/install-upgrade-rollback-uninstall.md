...
联网用户只下载统一安装器 ZIP，解压后双击 `install.cmd`。若同目录没有总清单，入口只从版本锁定的 `v0.12.0-rc.3` HTTPS Release URL 获取它，先校验 schema、产品和精确版本，再下载缺失资产；不使用 `latest`。完全离线时，把总清单和另外四个核心组件 ZIP 放在 `install.cmd` 同目录。仅在离线安装 Kokoro 时，才另行准备对应的 JSON 清单与全部分卷文件。本版本的三套 Kokoro 运行包内容未变化，继续从已校验的公开 `v0.10.0-rc.1` Release 获取。
