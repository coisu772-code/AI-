...
当前预发布版本是 `v0.11.0-rc.4`。本版在 `v0.11.0-rc.3` 的三档迁移机制上补齐来源证据驱动的完整仿写链：视频或文本仿写固定经过来源包与完整拆解，15 个方向逐项绑定来源事实和自然扩展缺口，并在 `D2_DECONSTRUCTION` 等待用户选择。系统不会代替用户执行 Google/YouTube OAuth 或真实视频上传。
