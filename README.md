...
当前预发布候选版本是 `v0.14.0-rc.1`。本版把 Codex 视觉计划升级为 schema 1.5，新增按视觉语义合并配音行、角色生命周期外观绑定、战斗关键瞬间约束、分类音效时长、失败步骤选择性续跑和更强的多方向动态镜头；配套新漫剧工坊版本为 `2.7.0-rc.1`。统一安装器与全部组件锁定到本版本，系统不会代替用户执行 Google／YouTube OAuth。
