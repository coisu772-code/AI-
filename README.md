...
新内容主链为：`content-source` 只负责取得并保存视频字幕、上传文本、资料库或按需联网资料；用户确认创作简报／故事大纲后，`content-rewrite` 生成完整正文初稿，`content-review-edit` 审核修改并冻结正式文本，`content-title-description` 再生成标题、YouTube 简介与正式封面。旧拆解 Skill、旧视频拆文工具、迁移／仿写方向推荐和 8 方向仿写工具均已从新任务入口移除。
...
每个项目只为本次实际执行的创作、审核、包装与制作阶段生成 `用户审核文档`，不再固定补齐 `01`–`11`，也不生成或要求独立的“原始素材说明”。稳定文件名指向最新版本，历史版本、路径、大小和 SHA-256 继续保留。中文版和中文包装翻译只供审核，不会进入配音、字幕、分镜或 YouTube 正式字段。
