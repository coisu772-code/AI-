...
2. 调用 `content_workspace_document_save` 分别保存完整审核报告、修改记录和修改后全文；审核报告、修改记录、中文审核稿与外语质量记录统一使用 `confirmationRequired=false`，仍完整展示但不产生重复确认门。报告至少包含问题位置、级别、证据、影响和修改建议，修改记录必须列明修改前后及原因。
...
7. 确认卡先展示中文版路径和质量结论，再展示目标语言正式稿路径、版本与 SHA-256；只有目标语言正式稿使用 `confirmationRequired=true`。用户确认后调用 `content_workspace_document_confirm` 冻结该版本；其哈希未变化时不得再要求第二次确认。
