...
本合同只适用于用户本次明确选择的 `director` 精品导演模式。`fast_auto` 极速自动模式与 `balanced` 平衡模式不得读取本合同、不得生成 `codexVisualPlan`；平衡模式的图片提示词由工坊现有分析步骤生成。精品导演模式下，用户只选择视频提示词开关与实际镜头视频范围，不需要自己编写提示词；实际镜头视频仍是独立开关，只开启视频提示词不能开启视频生成。
