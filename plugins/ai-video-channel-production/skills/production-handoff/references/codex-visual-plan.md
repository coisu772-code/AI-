...
- `referenceSheetPromptZh`：单角色漫画设定图提示词，禁止剧情动作和背景文字；
...
角色参考图固定为 `identity_only`。只锁定身份，不锁定表情、视线、头部角度、身体姿势、手势、构图、光线或背景。
...
`scenePlans` 必须按正式稿顺序完整覆盖每个 `lineId` 一次且仅一次，可以把连续且属于同一可视动作／情绪节拍的行组合成一个镜头，但不能跨集。默认每镜 1–2 行，并结合文本长度控制在约 3.5–7 秒；一个镜头出现动作、地点或主要情绪切换时必须拆开。每镜先写表演合同，再写图片提示词：
