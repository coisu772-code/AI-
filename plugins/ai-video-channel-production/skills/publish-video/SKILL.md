...
8. 显示项目、发布意图、频道序号、策略、隐私、计划、视频参数、标题、简介、Hashtags、封面、字幕、验证结论、正式发布中心状态和下一动作。
...
- `AUTO`：验证工作区授权、频道 AUTO 授权、当前发布意图 AUTO 授权及其时间／版本。缺一项就停在 `WAITING_REVIEW`；全部齐全后移交正式发布中心。只有发布中心自身保存的全局自动发布同意、频道 AUTO、有效 OAuth 与额度门也全部通过时，发布中心才可开始真实上传。Codex 不读取 Token，也不代替发布中心授权门。
...
- Codex 桥接调用始终保持 `networkExecution=false`；真实上传只能由独立的 YouTube 发布中心在既有明确授权门通过后执行。不得把桥接器的离线边界写成整个产品 `upload=false`。
