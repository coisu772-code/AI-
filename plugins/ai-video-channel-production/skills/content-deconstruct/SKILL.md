...
- 每个方向必须包含改编档位、来源核心保留说明、开端—升级—关键转折—高潮功能—结局回报的完整因果骨架、原创实现和风险；仍只到方向骨架，不生成正式大纲、场景、对白或正文。
...
同时冻结 `adaptationProfiles`、`preservationContract`、`directionDistinctnessMatrix`、每个方向的 `sourceFidelityEvidence` 与 `nonCopyEvidence`，供仿写质量门复核。
