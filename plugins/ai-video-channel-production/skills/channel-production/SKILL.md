...
1. 用户发送视频链接、文章链接、本地文件或正文，并说“下载字幕、拆书、拆解、分析文案、完全仿写、仿写、给仿写方向”时，固定走 `$content-source → $content-deconstruct`。不得改走“素材研究与原创重构”，也不得从标题、简介或压缩摘要直接生成新故事。
2. 用户明确说“原创重构”或“本土化扩写”时，才允许进入独立的素材研究前置模块；该模块产物必须明确标注为“原创重构”，不能冒充高贴合仿写方向。
3. 用户只发送链接、没有说明要拆解仿写还是原创重构时，显示一张二选一卡并停止：`1 拆解并仿写（推荐）`、`2 原创重构`。不得自行选择第 2 项。
4. 来源取得后必须保留完整规范正文及段落／时间证据索引。下游拆解只能读取 Source Package，不能把整份字幕先压缩成卖点摘要再据此生成方向。
5. 多组选项使用互不冲突的编号，例如入口用数字、频道预设用字母。用户回复的编号不存在或可作多种解释时先澄清，不得静默换算成“第一个预设”。
6. 拆解完成必须显示 `02_完整拆解报告.md` 与 `03_迁移方向选择.md` 并停在 `D2_DECONSTRUCTION`。没有本次自动授权时，禁止直接生成大纲或正文。

## 路由

- 建立／进入频道资料库、修改频道默认值、备份恢复：`$channel-onboarding`。
- 视频链接、用户文件、粘贴文本、资料库检索或用户明确要求的公开联网资料：`$content-source`；出现“拆解／仿写”意图时取得来源后固定交给 `$content-deconstruct`。
