...
param(
    [string]$ManifestPath,
    [string]$ManifestUrl = "https://github.com/coisu772-code/AI-/releases/download/v0.16.0-rc.1/unified-release-v0.16.0-rc.1.json",
...
$null = Assert-AivcpPathBudget (Join-Path $installFull "downloads\manifest-cache\unified-release-v0.16.0-rc.1.json") "locked manifest cache"
...
if ([string]::IsNullOrWhiteSpace($ManifestPath)) {
    $localManifest = Join-Path $assetFull "unified-release-v0.16.0-rc.1.json"
    if (Test-Path -LiteralPath $localManifest -PathType Leaf) {
        $ManifestPath = $localManifest
    }
    else {
        if ($InstallMode -eq "Offline") { throw "Offline installation requires unified-release-v0.16.0-rc.1.json beside install.cmd." }
        ...
        $ManifestPath = Join-Path $manifestCache "unified-release-v0.16.0-rc.1.json"
...
if ([string]$manifest.productVersion -ne "0.16.0-rc.1" -or @($manifest.assets).Count -ne 5 -or $null -eq $manifest.runtime -or $null -eq $manifest.safetyBoundaries) {
    throw "The downloaded manifest does not match the locked v0.16.0-rc.1 schema and product version."
}
